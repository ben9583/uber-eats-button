...
const prompt =
  'Here are a list of food items with their prices from the restaurant "{0}". Please create a tasty, complete, modest meal for one person under $50 out of the following menu items. If relevant, try to avoid picking anything spicy and don\'t get water as a drink. I\'ve included the prices in parentheses and some of the items have a description. Provide the items I should get using just the names I gave you, separated by newlines, with no other formatting or text.\n\n{1}';
/**
...
const selectItems = async (restaurant, items) => {
  ...
  const thisPrompt = prompt
    .replace("{0}", restaurant)
    .replace("{1}", formattedItems);
